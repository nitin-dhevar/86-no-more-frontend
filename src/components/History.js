...
class History extends Component {
    ...
    render() {
        ...
        const datacost = ['']
        job.map(
            (job) => job.restname == user.restname && labels.push(job.itemname)
        )
        job.map(
            (job) => job.restname == user.restname && data.push(job.quantity)
        )
        job.map(
            (job) =>
                job.restname == user.restname && datacost.push(job.costperitem)
        )
        return (
            <div>
